...
class Agent(object):
    ...
    def _get_revenue(self, x, price0, price1):
        ...
        lower0_proba = self.item0_model.predict_proba(np.concatenate((x, [price0])).reshape(1, -1))
        lower1_proba = self.item1_model.predict_proba(np.concatenate((x, [price1])).reshape(1, -1))
        revenues = (outcome_proba[0][1] * price0 * lower0_proba[0][1] + outcome_proba[0][2] * price1 * lower1_proba[0][1])
        return revenues
    ...
    def _process_last_sale(self, last_sale, profit_each_team):
        if type(self.last_cavariates) == type(None):
            return
        ...
        X_train, Y_train = self._generate_data(opponent_last_prices, self.last_cavariates, 0)
        if len(X_train) > 0:
            random.shuffle(X_train)
            random.shuffle(Y_train)
            self.item0_model.partial_fit(X_train, Y_train)
        X_train, Y_train = self._generate_data(opponent_last_prices, self.last_cavariates, 1)
        if len(X_train) > 0:
            random.shuffle(X_train)
            random.shuffle(Y_train)     
            self.item1_model.partial_fit(X_train, Y_train)
